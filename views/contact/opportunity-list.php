...
<div class="row relatedListStart">
	<div style="font-size: 10pt"><?= Yii::t('main', 'Opportunities') ?></div>
	<a href="<?= Url::to(['opportunity/create', 'contact_id'=>$model->id]) ?>" class="btn btn-primary btn-xs"><?= Yii::t('main', 'Add New') ?></a>
</div>
...
if (count($model->opportunitycontacts))
  foreach ($model->opportunitycontacts as $contact)
  {
    $record=$contact->opportunity;
